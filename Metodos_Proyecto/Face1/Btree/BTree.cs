namespace Metodos_Proyecto
{
    ...
    public class BTree<TK, TP> where TK : IComparable<TK>
    {
        ...
        public Entry<TK, TP> SearchDPI(TK key)
        {
            return this.SearchInternalDPI(this.Root, key);
        }
        ...
        private Entry<TK, TP> SearchInternalDPI(Node<TK, TP> node, TK key)
        {
            int i = node.Entries.TakeWhile(entry => key.CompareTo(entry.Key) > 0).Count();
            ...
            return node.IsLeaf ? null : this.SearchInternalDPI(node.Children[i], key);
        }
        /// <summary>
